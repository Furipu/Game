...
module.exports = class World {
  ...
  addPlayer(name) {
    ...
    this.allPlayersMap.set(
      this.makeKey(playerPosition.xPos, playerPosition.yPos),
      player
    );
    return {
  ...
  timeOutFights() {
    ...
    setInterval(() => {
      timeToCheck = Date.now() - 10000;
      for (const fight of Object.values(this.allFightsMap)) {
        if (fight.timestamp < timeToCheck) {
          if (fight.playerFrom.weapon === "")
            this.allPlayersMap.delete(fight.playerFrom);
          if (fight.playerTo.weapon === "")
            this.allPlayersMap.delete(fight.playerTo);
          return this.allPlayersMap;
        }
      }
      console.log(`${new Date(Date.now())} - ${timeToCheck}`);
    }, 10000);
